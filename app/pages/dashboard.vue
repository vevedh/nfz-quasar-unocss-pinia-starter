  ...
  <div class="grid gap-6">
    ...
    <section class="grid gap-4 md:grid-cols-3">
      <QCard class="nfz-card nfz-card-hover">
        <QCardSection>
          <div class="flex items-center justify-between">
            <div>
              <div class="nfz-muted text-sm">Utilisateur</div>
              <div class="mt-1 text-xl font-800">{{ session.userLabel }}</div>
            </div>
      ...
      <QCard class="nfz-card nfz-card-hover">
        <QCardSection>
          <div class="flex items-center justify-between">
            <div>
              <div class="nfz-muted text-sm">Rôles</div>
              <div class="mt-1 text-xl font-800">{{ session.roles.join(', ') }}</div>
            </div>
      ...
      <QCard class="nfz-card nfz-card-hover">
        <QCardSection>
          <div class="flex items-center justify-between">
            <div>
              <div class="nfz-muted text-sm">Messages</div>
              <div class="mt-1 text-xl font-800">{{ messages.total }}</div>
            </div>
    ...
    <section class="nfz-panel">
      <div class="mb-4 flex items-center justify-between">
        <div>
          <h2 class="text-lg font-800">Derniers messages</h2>
          <p class="nfz-muted text-sm">Service Feathers protégé par JWT.</p>
        </div>
      ...
      <QList bordered separator class="rounded-xl overflow-hidden">
        <QItem v-for="message in messages.latest" :key="message.id">
          <QItemSection>
            ...
            <QItemLabel caption>{{ message.createdAt }}</QItemLabel>
          </QItemSection>
        </QItem>
        <QItem v-if="!messages.latest.length">
          <QItemSection>
            <QItemLabel class="nfz-muted">Aucun message pour l’instant.</QItemLabel>
          </QItemSection>
        </QItem>
      </QList>
    </section>
