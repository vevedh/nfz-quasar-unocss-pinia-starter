...
export default async function seedUsers(app: Application, _ctx: SeedContext = {}) {
  ...
  if (!shouldSeedDemoUser(runtimeConfig)) {
    console.warn('[nfz-starter] Demo seed disabled for this environment.')
    return
  }
  ...
  if (!rows.length) {
    ...
    console.warn('[nfz-starter] Demo seed user created.')
  }
  else {
    console.warn('[nfz-starter] Demo seed user already exists.')
  }
