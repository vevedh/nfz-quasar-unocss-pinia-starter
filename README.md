...
Le starter contient maintenant une documentation pédagogique et animée sous `docs/`.
...
- la création du starter ;
- le flow auth `useLocalAuthUi()` → `studioSession` → NFZ/Feathers ;
- le middleware session/RBAC ;
- la couche `useAdminFeathers()` ;
...
- GitHub Pages : <https://vevedh.github.io/nuxt-feathers-zod/>
...
## Installation Bun stable sous Windows

Ce starter contient un `bunfig.toml` qui force `install.concurrentScripts = 1`.
Ce réglage évite les échecs intermittents d'installation du binaire `esbuild`
observés dans les arbres Nuxt/Vite sous Windows, par exemple :

```txt
Error: Expected "0.27.7" but got ""
```
...
En cas d'installation déjà corrompue ou interrompue :

```bash
bun run install:clean:win
```

Commande portable équivalente :

```bash
bun install --concurrent-scripts=1
```

Les dépendances de documentation VitePress restent isolées dans `docs/package.json`.
Le `bun install` racine installe donc uniquement l'application Nuxt/NFZ, puis les
scripts `docs:*` installent les dépendances de documentation dans `docs/`.
